function [stars, stderror] = inference_flex(coef,y,X,W,Q,G)
...
if Q == 0
    ...
    grad = information_mat_flex(coef, y, X, W, Q, G);
    ...
    cov_matrix = inv(grad);
    intervals_HO1 = zeros(Q+K+1,2);
    intervals_HO2 = zeros(Q+K+1,2);
    intervals_HO3 = zeros(Q+K+1,2);
    intervals_HO1(:,1) = -sqrt(diag(cov_matrix)) *1.64 + coef';
    intervals_HO1(:,2) = sqrt(diag(cov_matrix)) *1.64+ coef';
    intervals_HO2(:,1) = -sqrt(diag(cov_matrix)) *1.96 + coef';
    intervals_HO2(:,2) = sqrt(diag(cov_matrix)) *1.96+ coef';
